...
def plotTimeseries0Dvs1D(pneuron, a, cov, rs, deff, drive, pp, figsize=(8, 6), fs=12):
    ...
    for k, df in ext_data.items():
        ext_data[k] = prependDataFrame(punctual_data, tonset=tonset) #prependDataFrame(df, tonset=tonset)
...
def plotTimeseries0D(pneuron, a, cov, rs, deff, drive, pp, figsize=(8, 6), fs=12):
...
def plotTimeseries1D(pneuron, a, cov, rs, deff, drive, pp, figsize=(8, 6), fs=12):

    # Simulate extended SONIC model with specific membrane coverage
    ext_model = surroundedSonophore(pneuron, a, cov, rs, depth=deff)
    ext_data, _ = ext_model.simulate(SectionAcousticSource('center', drive.f, drive.A), pp, dt=0.00005)
    ...
    # Get stimulus patches
    t = ext_data[k]['t'].values  # s
    stimon =  ext_data[k]['stimstate'].values
    pulse = CompTimeSeries.getStimPulses(t, stimon)[0]
